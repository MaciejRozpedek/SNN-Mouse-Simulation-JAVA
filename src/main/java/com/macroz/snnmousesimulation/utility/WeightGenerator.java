...
public class WeightGenerator {
    ...
    private WeightGenerator(GenerationType type, double p1, double p2, Random random) {
        this.type = type;
        this.p1 = p1;
        this.p2 = p2;
    ...
    public static WeightGenerator createFixed(double fixedValue) {
        return new WeightGenerator(GenerationType.FIXED, fixedValue, 0.0, new Random());
    }
    ...
    public static WeightGenerator createUniform(double min, double max) {
        ...
        return createUniform(min, max, new Random());
    }

    public static WeightGenerator createUniform(double min, double max, Random random) {
        if (min > max) {
            throw new IllegalArgumentException("Minimal value cannot be greater than maximal value in Uniform distribution.");
        }
        return new WeightGenerator(GenerationType.UNIFORM, min, max, random);
    }
    ...
    public static WeightGenerator createNormal(double mean, double std) {
        ...
        return createNormal(mean, std, new Random());
    }

    public static WeightGenerator createNormal(double mean, double std, Random random) {
        if (std < 0) {
            throw new IllegalArgumentException("Standard deviation cannot be negative.");
        }
        return new WeightGenerator(GenerationType.NORMAL, mean, std, random);
    }
